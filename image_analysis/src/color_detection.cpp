...
int LowerS = 0;
int LowerV = 0;
//int UpperH = 180; int UpperS = 196; int UpperV = 170;
int UpperH = 255;
int UpperS = 255;
int UpperV = 255;
/*
int LowerH = 0; int LowerS = 0; int LowerV = 0;
int UpperH = 180; int UpperS = 196; int UpperV = 170; */
...
void colorDetectionCallback(const sensor_msgs::ImageConstPtr &original_image)
{
    ...
    cv::inRange(cv_ptr->image, cv::Scalar(LowerH, LowerS, LowerV), cv::Scalar(UpperH, UpperS, UpperV), img_mask);
#endif
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    nh.param<std::string>("topic_image", topic_image, "/zed/rgb/image_rect_color");
    ROS_INFO_STREAM(topic_image);
...
#elif defined(USE_RGB)
    cv::namedWindow("Ball");
    cv::createTrackbar("LowerH", "Ball", &LowerH, 255, NULL);
    cv::createTrackbar("UpperH", "Ball", &UpperH, 255, NULL);
    cv::createTrackbar("LowerS", "Ball", &LowerS, 255, NULL);
    cv::createTrackbar("UpperS", "Ball", &UpperS, 255, NULL);
    cv::createTrackbar("LowerV", "Ball", &LowerV, 255, NULL);
    cv::createTrackbar("UpperV", "Ball", &UpperV, 255, NULL);
#endif
    ...
    */
    ...
    image_transport::Subscriber sub = it.subscribe(topic_image, 1, colorDetectionCallback);
